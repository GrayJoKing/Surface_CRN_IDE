...
export default class Custom_Regex_Decomposer {

	static decompose(init : string) {
		...
		if (brackets.length !== 0) throw "Unbalanced brackets";

		return this.parse(init);
	}

	static parse(s : string, matches = 1, curly_group = NaN) : string[] {
		...
		if (s === "") {
			...
			var t = XRegExp.matchRecursive(s, "\\{", "\\}\\??", 'g', {valueNames: ['between', 'left', 'match', 'right']});
